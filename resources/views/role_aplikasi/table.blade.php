...
<div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody>
                @forelse($roleaplikasis as $roleaplikasi)
                    <tr class="border-b border-white/5 hover:bg-white/5 transition-colors">
                        <td class="py-3 text-gray-300 text-sm">
                            <span class="bg-purple-500/20 text-purple-400 px-2 py-1 rounded-lg text-xs font-medium">
                                {{ $roleaplikasi->role->nama_role ?? 'N/A' }}
                            </span>
                        </td>
                        <td class="py-3 text-gray-300 text-sm">
                            <span class="bg-blue-500/20 text-blue-400 px-2 py-1 rounded-lg text-xs font-medium">
                                {{ $roleaplikasi->jenisAplikasi->jenis_aplikasi ?? 'N/A' }}
                            </span>
                            <span class="text-gray-400 text-xs ml-1">
                                ({{ $roleaplikasi->jenisAplikasi->deskripsi ?? 'N/A' }})
                            </span>
                        </td>
                        @if ($permissions['can_edit'] || $permissions['can_delete'])
                            <td class="py-3">
                                <div class="flex items-center gap-2">
                                    @if ($permissions['can_edit'])
                                        <button
                                            onclick="openEditModal('{{ $roleaplikasi->id }}', '{{ $roleaplikasi->role_id }}', '{{ $roleaplikasi->jenis_aplikasi_id }}', '{{ $roleaplikasi->role->nama_role ?? '' }}', '{{ $roleaplikasi->jenisAplikasi->jenis_aplikasi ?? '' }}')"
                                    ...
                                    @if ($permissions['can_delete'])
                                        <button
                                            onclick="openDeleteModal('{{ $roleaplikasi->id }}', '{{ $roleaplikasi->role->nama_role ?? '' }}', '{{ $roleaplikasi->jenisAplikasi->jenis_aplikasi ?? '' }}')"
                ...
                @empty
                    <tr>
                        <td colspan="{{ 2 + ($permissions['can_edit'] || $permissions['can_delete'] ? 1 : 0) }}"
                            ...
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <p>Tidak ada data role aplikasi</p>
                                @if ($permissions['can_create'])
                                    <p class="text-xs">Klik tombol "Tambah Role Aplikasi Baru" untuk menambahkan data
                                    </p>
                                @endif
                            </div>
                        </td>
    ...
    <!-- Pagination -->
    @if (method_exists($roleaplikasis, 'links') && $roleaplikasis->lastPage() > 1)
        <div class="mt-4 flex items-center justify-between">
            <p class="text-gray-400 text-sm">
                Menampilkan {{ $roleaplikasis->firstItem() ?? 0 }} - {{ $roleaplikasis->lastItem() ?? 0 }} dari
                {{ $roleaplikasis->total() }}
                data
            </p>
            <div class="flex items-center gap-2">
                {{-- Previous Button --}}
                @if ($roleaplikasis->onFirstPage())
                    <span class="px-3 py-1 bg-white/5 text-gray-500 rounded-lg text-sm cursor-not-allowed">←</span>
                @else
                    <a href="{{ $roleaplikasis->previousPageUrl() }}"
                ...
                @php
                    $totalPages = $roleaplikasis->lastPage();
                    $currentPage = $roleaplikasis->currentPage();
                ...
                @for ($i = $start; $i <= $end; $i++)
                    <a href="{{ $roleaplikasis->url($i) }}"
                        class="pagination-link px-3 py-1 {{ $roleaplikasis->currentPage() == $i ? 'bg-blue-500/20 text-blue-400' : 'bg-white/5 text-gray-300 hover:bg-white/10' }} rounded-lg text-sm">
                        {{ $i }}
                    </a>
                @endfor

                {{-- Next Button --}}
                @if ($roleaplikasis->hasMorePages())
                    <a href="{{ $roleaplikasis->nextPageUrl() }}"
                        class="pagination-link px-3 py-1 bg-white/5 text-gray-300 rounded-lg hover:bg-white/10 transition-colors text-sm">
                        →
                    </a>
                @else
