<!-- Additional Info: Roles Table -->
<div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
    <div class="overflow-x-auto">
        <table class="w-full">
            <thead>
                <tr class="border-b border-white/10">
                    <th class="text-left text-gray-400 text-xs font-medium py-3">Nama Role</th>
            ...
            <tbody>
                            ...
                            <td class="py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    @if ($permissions['can_delete'])
                                        <button
                                            onclick="openDeleteModal('{{ $role->id }}', '{{ $role->nama_role }}')"
                ...
                @empty
                    <tr>
                        <td colspan="{{ 1 + ($permissions['can_edit'] || $permissions['can_delete'] ? 1 : 0) }}"
                            ...
                            <div class="flex flex-col items-center gap-2">
                                ...
                                @if ($permissions['can_create'])
                                    <p class="text-xs">Klik tombol "Tambah Role Baru" untuk menambahkan data</p>
                                @endif
                            </div>
                        </td>
                    </tr>
    ...
    <!-- Pagination dengan 3 nomor berurutan (tanpa halaman 1 dan terakhir) -->
    @if (method_exists($roles, 'links') && $roles->lastPage() > 1)
        <div class="mt-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                @for ($i = $start; $i <= $end; $i++)
                    <a href="{{ $roles->url($i) }}"
                        class="pagination-link px-3 py-1 {{ $roles->currentPage() == $i ? 'bg-blue-500/20 text-blue-400' : 'bg-white/5 text-gray-300 hover:bg-white/10' }} rounded-lg text-sm">
                        {{ $i }}
                    </a>
                @endfor
