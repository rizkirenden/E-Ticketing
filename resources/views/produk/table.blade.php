...
<div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-white/10">
                    <th class="text-left text-gray-400 text-xs font-medium py-3">Kode Produk</th>
                    <th class="text-left text-gray-400 text-xs font-medium py-3">Nama Produk</th>
            ...
            <tbody>
                @forelse($produks as $produk)
                    <tr class="border-b border-white/5 hover:bg-white/5 transition-colors">
                        ...
                        <td class="py-3 text-gray-300 text-sm font-medium">{{ $produk->nama_produk }}</td>
                        <td class="py-3 text-gray-300 text-sm">
                            <span class="bg-purple-500/20 text-purple-400 px-2 py-1 rounded-lg text-xs font-medium">
                                {{ $produk->jenisAplikasi->kode_jenis_aplikasi ?? '-' }} -
                                {{ $produk->jenisAplikasi->jenis_aplikasi ?? '-' }}
                                <!-- PERBAIKI: tampilkan kode dan jenis aplikasi -->
                            </span>
                        </td>
                        <td class="py-3 text-gray-300 text-sm max-w-xs truncate" title="{{ $produk->deskripsi }}">
                            {{ $produk->deskripsi ?: '-' }}
                        </td>
                        @if ($permissions['can_edit'] || $permissions['can_delete'])
                            <td class="py-3">
                                <div class="flex items-center gap-2">
                                    @if ($permissions['can_edit'])
                                        <button
                                            onclick="openEditModal('{{ $produk->id }}', '{{ $produk->kode_produk }}', '{{ $produk->nama_produk }}', '{{ addslashes($produk->deskripsi) }}', '{{ $produk->jenis_aplikasi_id }}')"
                        ...
                        <td colspan="{{ 4 + ($permissions['can_edit'] || $permissions['can_delete'] ? 1 : 0) }}"
                            ...
                            <div class="flex flex-col items-center gap-2">
                                ...
                                @if ($permissions['can_create'])
                                    <p class="text-xs">Klik tombol "Tambah Produk Baru" untuk menambahkan data</p>
                                @endif
                            </div>
                        </td>
                    </tr>
    ...
    <!-- Pagination -->
    @if (method_exists($produks, 'links') && $produks->lastPage() > 1)
        <div class="mt-4 flex items-center justify-between">
            <p class="text-gray-400 text-sm">
                Menampilkan {{ $produks->firstItem() ?? 0 }} - {{ $produks->lastItem() ?? 0 }} dari
                {{ $produks->total() }}
                data
            </p>
            <div class="flex items-center gap-2">
                ...
                @for ($i = $start; $i <= $end; $i++)
                    <a href="{{ $produks->url($i) }}"
                        class="pagination-link px-3 py-1 {{ $produks->currentPage() == $i ? 'bg-blue-500/20 text-blue-400' : 'bg-white/5 text-gray-300 hover:bg-white/10' }} rounded-lg text-sm">
                        {{ $i }}
                    </a>
                @endfor
